...
🔧 Next Maintenance: {result.get('next_maintenance', 'N/A')}"""
    ...
    yield FunctionInfo.from_fn(
        _grid_optimize,
        description=("Optimizes power grid configuration for a specified region to minimize power losses. "
...
🔧 Next Scheduled: {status.get('next_scheduled', 'Not scheduled')}"""
    ...
    yield FunctionInfo.from_fn(
        _grid_status,
        description=("Retrieves current grid status, optimization history, and health metrics "
...
🕒 Timestamp: {analysis_result.get('timestamp', 'N/A')}"""
    ...
    yield FunctionInfo.from_fn(
        _grid_analyze,
        description=("Performs detailed analysis of grid performance metrics including efficiency, "
...
📅 Timestamp: {result.get('timestamp', 'N/A')}"""
    ...
    yield FunctionInfo.from_fn(
        ...
        description=("Legacy grid optimization function for backward compatibility. "
...
async def legacy_show_last_optimization(tool_config: LegacyShowOptimizationConfig, builder: Builder):
...
🔧 Next Scheduled: {result.get('next_scheduled', 'Not scheduled')}"""
    ...
    yield FunctionInfo.from_fn(
        ...
        description=("Legacy function to show last optimization results. "
