...
async def get_grid_status(region: str):
    ...
    if region not in REGION_NAMES:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid region '{region}'. Available regions: {REGION_NAMES}"
        )
...
async def get_optimization_history(region: str, limit: int = 10):
    ...
    try:
        ...
        history_entry = OptimizationHistory(
            ...
            efficiency=(1 - result.get('losses', 0) / result.get('optimized_supply', 1)) * 100 if result.get('optimized_supply', 0) > 0 else None,
