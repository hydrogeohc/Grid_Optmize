...
from .db import GridState, OptimizationResult, get_engine, get_session
...
async def get_optimization_status(region: str = "default") -> Dict[str, Any]:
    ...
    try:
        ...
        latest_state = (
            ...
            .filter(GridState.region == region)
        ...
        recent_optimizations = (
            ...
            .filter(OptimizationResult.region == region)
        ...
        status_data = {
            "current_status": (
                "Operational" if efficiency and efficiency > 80 else "Attention Required"
            ),
            "region": region,
            "current_load": getattr(latest_state, "current_load", latest_state.demand),
            "total_capacity": getattr(
                latest_state, "capacity", latest_state.supply * 1.2
            ),  # Mock capacity
...
async def analyze_grid_metrics(
    region: str = "default", metric: str = "efficiency"
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if metric.lower() == "efficiency":
            ...
            if efficiencies:
                ...
                trend = (
                    "improving"
                    if len(efficiencies) > 1 and efficiencies[0] > efficiencies[-1]
                    else "stable"
                )
            ...
            loads = [
                state.current_load or state.demand
                for state in historical_data
                if state.current_load or state.demand
            ]
            if loads:
